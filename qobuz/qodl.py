...
def parse_prefs(cfg, tag_cfg):
	if cfg:
		...
		parser.add_argument(
			'-f', '--file',
			default='',
			required='--url' not in sys.argv and '-u' not in sys.argv,
			help='File with URLs. Supported: album, artist page, fav albums, fav tracks, playlist, track.')
		parser.add_argument(
			'-u', '--url',
			default='',
			required='--file' not in sys.argv and '-f' not in sys.argv,
			help='URL. Supported: album, artist page, fav albums, fav tracks, playlist, track.')
		parser.add_argument(
			'-t', '--a_tags',
			default=cfg['album_tags'],
			action='store_true',
			help='Add HR (HiRes) and E (Explicit) in brackets at the beginning of the album folder.')
		parser.add_argument(
			'-i', '--a_id',
			default=cfg['album_id'],
			action='store_true',
			help='Add the album id to the end of the folder.')
		parser.add_argument(
			'-y', '--a_year',
			default=cfg['album_year'],
			action='store_true',
			help='Add the album year to the end of the folder.')
		parser.add_argument(
			'-q', '--quality',
			default=cfg['qual'],
		...
		cfg['file'] = args.file
		cfg['url'] = args.url
		cfg['album_tags'] = args.a_tags
		cfg['album_id'] = args.a_id
		cfg['album_year'] = args.a_year
		...
		rt_opts={
			...
			"album_tags": cfg['prefs']['album_tags'],
			"album_year": cfg['prefs']['album_year'],
			"album_id": cfg['prefs']['album_id'],
		...
		tags={
			...
			"ITUNESADVISORY": cfg['prefs']['tags']['advisory']}
	qual = rt_opts['qual']
...
def parse_meta(src, meta, num, tot):
	if meta:
		...
		meta['ARTIST'] = [str(src.get('performer', {}).get('name')).strip()]
		meta['ISRC'] = src.get('isrc')
		meta['DISCNUMBER'] = src.get('media_number')
		meta['COMPOSER'] = src.get('composer', {}).get('name')
		meta['TITLE'] = title
		meta['TRACKNUMBER'] = tracknum
		meta['PERFORMER'] = src.get('performer', {}).get('name')
		meta['ITUNESADVISORY'] = 1 if src.get('parental_warning') else 0

		performers = str(src.get('performers')).split(' - ')
		featurings = []
		for p in performers:
			plist = p.split(',')
			person = str(plist[0]).strip()
			credits = [re.sub(r"\s+", "", str(c)) for c in plist[1:]]
			if person not in meta['ARTIST']:
				if 'MainArtist' in credits:
					meta['ARTIST'].append(person)
				elif 'FeaturedArtist' in credits:
					featurings.append(person)
		if featurings: meta['ARTIST'].extend(featurings)
	else:
		#print(src)
		if tot:
			tracktot = tot
		...
		try:
			year = datetime.fromtimestamp(src.get('released_at')).strftime('%Y-%m-%d')
		except OSError:
			try:
				year = str(src.get('release_date_original'))
		...
		meta={
			...
			"DATE": year,
			"YEAR": year,
			"ITUNESADVISORY": src.get('parental_warning'),
			"HIRES": src.get('hires_streamable'),
			"ID": src.get('id')}
	return meta
...
def download(id, album_fol_s, meta, num, tot, fol, qual, cov, ref, tag_cfg, fn_template, cov_dir, embed_cov):
	if not fn_template.strip():
		fn_template = ('0' if 0 < num < 10 else '') + str(num) + " " +  meta['TITLE'] + (' [Explicit]' 
		if meta['ITUNESADVISORY'] == 1 and tag_cfg['ITUNESADVISORY'] else '')
	else:
...
def main(type, id, cfg, tag_cfg, fol, cli):
	...
	album_tags = cfg['album_tags']
	album_year = cfg['album_year']
	...
	while True:
		...
		if type == "album":
			...
			tags = ('HR' if al_meta['HIRES'] else '') + str('E' if al_meta['ITUNESADVISORY'] else '') if album_tags else ''
			album_fol = str('[' + tags + '] ' if tags.strip() else '') + al_meta['ALBUM'] + \
				(' [' + al_meta['ID'] + ']' if album_id else '') + (' [' + al_meta['YEAR'].split('-')[0] + ']' if album_year else '')
			print(album_fol + "\n")
			del al_meta['HIRES']
			del al_meta['ID']
			if fol is not None:
				album_fol_s = os.path.join(dl_dir, fol, sanitize(al_meta['ALBUMARTIST']), sanitize(album_fol))
			else:
				album_fol_s = os.path.join(dl_dir, sanitize(al_meta['ALBUMARTIST']), sanitize(album_fol))
	...
	try:
		...
		try:
			if sys.argv[1]:
				...
				file_path = cfg['file'] if 'file' in cfg else None
				cli = True
		except IndexError:
			cli = False
		...
		if not file_path:
			...
			with open(file_path, 'r') as urls_file:
				for line in urls_file.readlines():
					if str(line).startswith('https://'):
						cfg['url'] = line.strip()
						try:
							main(False, False, cfg, tag_cfg, None, cli)
							with open(f"/{'/'.join(file_path.split('/')[:-1])}/downloaded.txt", 'a+') as down_file:
								down_file.write(line)
						except KeyboardInterrupt:
							print("\n\nAdiós...")
							sys.exit()
						except:
							print(f"Error con: {line}")
							with open(f"/{'/'.join(file_path.split('/')[:-1])}/errors.txt", 'a+') as errors_file:
